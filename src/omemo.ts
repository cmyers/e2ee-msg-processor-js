...
export async function encryptMessage(session: libsignal.SessionCipher, plaintext: string): Promise<OmemoMessage> {
...
export async function decryptMessage(session: libsignal.SessionCipher, omemoMessage: OmemoMessage): Promise<string> {
...
export function generateIdentity(store: SignalProtocolStore) {
    return Promise.all([
...
export function generatePreKeyBundle(store: SignalProtocolStore) {
